...
<div class="app-container">
	<div class="row content-container">
		...
		<form action="${pageContext.request.contextPath }/competition/XiuGaiJingSai" method="post" onsubmit='return check();'>			
		<div class="container-fluid">
			<div class="side-body">
				<div class="page-title"> <span class="title">修改竞赛</span>
					<div class="description">在此页面修改竞赛，并修改竞赛基本信息</div>
				</div>
				<div class="row">
					<div class="col-xs-12">						
						<div class="card">
							...
							<!-- 开始 -->
							<c:forEach items="${requestScope.competitioninfo}" var="compinfo">
							<div class="card-body">
								<div class="sub-title">竞赛名称</div>
								<div>									
									<input type="text" class="form-control" value="${compinfo.compname }" name="Compname">									
								</div>
								
								<div class="sub-title">竞赛编号</div>							
								<div>									
									<input type="text" class="form-control" value="${compinfo.compid }" name="Compid" id="cid">
									<input type="hidden" value="${compinfo.id }" name="Id">
								</div>
								
								<div class="sub-title">申办单位</div>								
								<div>
									<select  name="Sponsor" id="selectsponsor">
										<optgroup label="申办单位">
										<option value="金融系" <c:if test="${compinfo.sponsor=='金融系'}">selected</c:if>>金融系</option>
										<option value="会计系" <c:if test="${compinfo.sponsor=='会计系'}">selected</c:if>>会计系</option>
										<option value="经贸系" <c:if test="${compinfo.sponsor=='经贸系'}">selected</c:if>>经贸系</option>
										<option value="管理系" <c:if test="${compinfo.sponsor=='管理系'}">selected</c:if>>管理系</option>
										<option value="国际教育学院" <c:if test="${compinfo.sponsor=='国际教育学院'}">selected</c:if>>国际教育学院</option>
										<option value="国际金融服务外包学院" <c:if test="${compinfo.sponsor=='国际金融服务外包学院'}">selected</c:if>>国际金融服务外包学院</option>
										<option value="信管系" <c:if test="${compinfo.sponsor=='信管系'}">selected</c:if>>信管系</option>
										<option value="商务外语系" <c:if test="${compinfo.sponsor=='商务外语系'}">selected</c:if>>商务外语系</option>
										<option value="保险系" <c:if test="${compinfo.sponsor=='保险系'}">selected</c:if>>保险系</option>
										<option value="法律系" <c:if test="${compinfo.sponsor=='法律系'}">selected</c:if>>法律系</option>
										<option value="校团委" <c:if test="${compinfo.sponsor=='校团委'}">selected</c:if>>校团委</option>
										<option value="基础课教学部" <c:if test="${compinfo.sponsor=='基础课教学部'}">selected</c:if>>基础课教学部</option>
										<option value="公共外语教学部" <c:if test="${compinfo.sponsor=='公共外语教学部'}">selected</c:if>>公共外语教学部</option>
										<option value="研究生部" <c:if test="${compinfo.sponsor=='研究生部'}">selected</c:if>>研究生部</option>
										<option value="学生处" <c:if test="${compinfo.sponsor=='学生处'}">selected</c:if>>学生处</option>
										</optgroup>
									</select><span><b><font color="#FF0000 ">（必选）</font></b></span>
								</div>
								<div class="sub-title">立项时间</div>
								<div>
									<input type="month" class="form-control" value="${compinfo.starttime }" name="Starttime" id="st">
								</div>
								<div class="sub-title">结项时间</div>
								<div>
									<input type="month" class="form-control" value="${compinfo.endtime }" name="Endtime" id="et">
								</div>
								...
								<div>
									<textarea type="text" rows="3" class="form-control" name="Remark" >${compinfo.remark }</textarea>
								</div>
								...
								<!-- 注释 -->
								<!-- </form action="uplaod.do" method="post" enctype="multipart/form-data">
								<form action=>
								<div class="sub-title">导入竞赛数据</div>
								<div class="form-group">
									<label for="exampleInputFile">请严格按照模板套表填写，限支持.xls格式</label>
									<input type="file" id="exampleInputFile" name="file">
								</div>
								<button type="submit" class="btn btn-info" formmethod="post" formaction="uplaod.do" formenctype="multipart/form-data">提交数据</button>
								</form> -->
							</div>
							</c:forEach>
							<!-- 结束 -->
