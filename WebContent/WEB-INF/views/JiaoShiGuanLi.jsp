...
<div class="app-container">
	<div class="row content-container">
		...
		<div class="container-fluid">
			<div class="side-body">
				<div class="page-title"><span class="title">参赛教师管理</span>
					<div class="description">在此界面查看、管理参赛教师信息</div>
				</div>
				<div class="row">
					<div class="col-xs-12">
						<div class="card">
							...
							<c:if test="${empty requestScope.trinfomana}">
								<h4 style="display:block;text-align:center">没有教师信息!</h4>
							</c:if>
							...
							<div class="card-body">
								<table class="datatable table table-striped" cellspacing="0" width="100%">
									...
									<%int i=1; %>
									<!-- 注释 JSTL 循环 -->
									<c:forEach items="${requestScope.trinfomana}" var="trinfomana">
										<tr>
											<td><%=i %></td>
											<td><a href="${pageContext.request.contextPath }/teacher/TzdJiaoShiXinXi?Trid=${trinfomana.trid }&Trname=${trinfomana.trname }">${trinfomana.trid }</a></td>
											<td><a href="${pageContext.request.contextPath }/teacher/TzdJiaoShiXinXi?Trid=${trinfomana.trid }&Trname=${trinfomana.trname }">${trinfomana.trname }</a></td>
											<td><a>${trinfomana.trdepartment }</a></td>
											<td>${trinfomana.trTpartnum }</td>
											<td>${trinfomana.totalworkload }</td>
											<td><a href="${pageContext.request.contextPath }/teacher/TzXiuGaiTrInfo?Trid=${trinfomana.trid }">查看/修改</a></td>
											<td><a href="${pageContext.request.contextPath }/teacher/ShanChuJiaoShi?Trid=${trinfomana.trid }">删除</a></td>
										</tr>
										<%i=i+1; %>
									</c:forEach>	
									<!-- 注释 JSTL 循环 -->
							...
							<c:if test="${empty requestScope.singletrinfo}">
								<h4 style="display:block;text-align:center"><span><font color="red">没有该教师的信息!</font></span></h4>
							</c:if>
							...
							<div class="card-body">
								<table class="datatable table table-striped" cellspacing="0" width="100%">
									...
									<%int i=1; %>
									<!-- 注释 JSTL 循环 -->
									<c:forEach items="${requestScope.singletrinfo}" var="singletrinfo">
										<tr>
											<td><%=i %></td>
											<td><a href="${pageContext.request.contextPath }/teacher/TzdJiaoShiXinXi?Trid=${singletrinfo.trid }&Trname=${singletrinfo.trname }">${singletrinfo.trid }</a></td>
											<td><a href="${pageContext.request.contextPath }/teacher/TzdJiaoShiXinXi?Trid=${singletrinfo.trid }&Trname=${singletrinfo.trname }">${singletrinfo.trname }</a></td>
											<td><a>${singletrinfo.trdepartment }</a></td>
											<td>${singletrinfo.trTpartnum }</td>
											<td>${singletrinfo.totalworkload }</td>
											<td><a href="${pageContext.request.contextPath }/teacher/TzXiuGaiTrInfo?Trid=${singletrinfo.trid }">查看/修改</a></td>
											<td><a href="${pageContext.request.contextPath }/teacher/ShanChuJiaoShi?Trid=${trinfomana.trid }">删除</a></td>
										</tr>
										<%i=i+1; %>
									</c:forEach>	
	...
	<footer class="app-footer">
		<div class="wrapper"> <span class="pull-right">2.1 <a href="#"><i class="fa fa-long-arrow-up"></i></a></span> Â© 2015 Copyright. </div>
	</footer>
