...
<head>
	...
	function check(){
		...
		var re2 = /^\d+(\.\d+)?$/; // 非负浮点数（正浮点数 + 0） // 暂时未用
		
		if(cre == null||re1.test(cre)==false){
			alert("请重新输入认定学分！"); 
			return false;
		}else{
			if (bon == null||re1.test(bon)==false) { 
				alert("请重新输入认定奖金！"); 
				return false; 
			}else{
				if(wor == null||re1.test(wor)==false){
					alert("请重新输入认定工作量！");
					return false;
				}else{
					return true;
...
<div class="app-container">
	<div class="row content-container">
		...
		<form action="${pageContext.request.contextPath }/criteria/TzdXiuGaiCriteria" method="post" onsubmit='return check();'>			
		<div class="container-fluid">
			<div class="side-body">
				<div class="page-title"> <span class="title">修改评定标准</span>
					<div class="description">在此页面修改竞赛的评定标准</div>
				</div>
				<div class="row">
					<div class="col-xs-12">						
						<div class="card">
							<div class="card-header">
								<div class="card-title">
									<div class="title">评判信息</div>
								</div>
							</div>
							<!-- 开始 -->
							<c:forEach items="${requestScope.updatejudgcri}" var="updatejudgcri">
							<div class="card-body">
								<div class="sub-title">获奖等级</div>
								<div>									
									<input type="text" class="form-control" value="${updatejudgcri.award }" name="Award">									
								</div>
								
								<div class="sub-title">校内认定</div>							
								<div>									
									<input type="text" class="form-control" value="${updatejudgcri.affirmation }" name="Affirmation">
									<input type="hidden" value="${updatejudgcri.id }" name="Id">
								</div>
								
								<div class="sub-title">认定学分（个人）</div>	
									<input type="text" class="form-control" value="${updatejudgcri.cricredit }" name="Cricredit" id="cre">																
								<div>
								
								</div>
								<div class="sub-title">认定奖金（个人）</div>
								<div>
									<input type="text" class="form-control" value="${updatejudgcri.cribonus }" name="Cribonus" id="bon">
								</div>
								<div class="sub-title">认定工作量（个人）</div>
								<div>
									<input type="text" class="form-control" value="${updatejudgcri.criworkload }" name="Criworkload" id="wor">
								</div>
